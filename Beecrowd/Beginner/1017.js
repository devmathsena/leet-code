...
const carConsumKmLiter = 12.0;
const timeSpentHours = parseInt(lines[0]);
const averageSpeed = parseInt(lines[1]);
const distanceToTravel = averageSpeed * timeSpentHours;

function calcLitersNeeded(distanceToTravel, carConsumKmLiter) {
    const litersNeeded = distanceToTravel / carConsumKmLiter;
    return litersNeeded.toFixed(3);
}

const litersNeeded = calcLitersNeeded(distanceToTravel, carConsumKmLiter);
